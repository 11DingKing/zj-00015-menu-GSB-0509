<template>
  <div>
    <div class="page-toolbar">
      ...
      <button 
        v-if="userStore.hasButtonPermission('order:add')" 
        class="btn btn-primary"
        @click="openCreateModal"
      >新增订单</button>
      <button 
        v-if="userStore.hasButtonPermission('order:export')" 
        class="btn btn-default"
      >导出订单</button>
    </div>

    <div v-if="meta" class="meta-info" style="margin: 16px 0;">
      <span class="tag tag-info">数据范围: {{ getDataScopeLabel(meta.dataScope) }}</span>
      <span v-if="meta.fieldMaskingApplied" class="tag tag-warning">字段脱敏已启用</span>
    </div>
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="order in orders" :key="order.id">
            ...
            <td>
              <button 
                v-if="userStore.hasButtonPermission('order:edit')"
                class="btn btn-default btn-sm" 
                style="margin-right: 4px;"
              >编辑</button>
              <button 
                v-if="userStore.hasButtonPermission('order:delete')"
