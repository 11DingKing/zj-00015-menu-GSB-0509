...
export default async function userRoutes(fastify: FastifyInstance) {
  fastify.get(
    ...
    async (request, reply) => {
      ...
      const nextCursor =
        data.length === take ? data[data.length - 1].id : undefined;
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const operator = await fastify.prisma.user.findUnique({
  ...
  fastify.delete(
    ...
    async (request, reply) => {
      ...
      if (params.id === user.userId) {
        return reply
          .code(400)
          .send({ success: false, message: "不能删除自己" });
      }
  ...
  fastify.post(
    "/api/users/:id/permissions",
    {
      ...
      schema: {
        tags: ["用户管理"],
        description:
          "用户直接授权（需要 system:user:grantPerm 权限，越过角色直接授权）",
      },
    },
    async (request, reply) => {
      const params = request.params as any;
